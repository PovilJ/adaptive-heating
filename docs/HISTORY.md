...
Last reviewed: **2026-09-24**. Current source version: **0.2.0, experimental**; first-house installed version: **0.1.0**.
...
| HA compatibility | Isolated HA lifecycle validated | 0.2.0 tests cover real HA 2026.7.4 setup, platform/entity creation, options save/reload, and unload with simulated tank-target restoration. Hardware/network behavior still needs field validation. |
...
Before enabling tank scheduling, update to 0.2.0, configure tank mappings and
...
legacy sun/time strategies still requires a scope decision and validation. The
disinfection rewrite is implemented in 0.2.0, with field validation still pending.
