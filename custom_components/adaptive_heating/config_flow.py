...
def settings_schema(values):
    ...
    for key, domains in (OPTIONAL_ENTITIES | DHW_ENTITIES).items():
        marker = vol.Optional(key, description={"suggested_value": values.get(key)})
        fields[marker] = selector.EntitySelector(selector.EntitySelectorConfig(domain=domains))
    for key, default in (DEFAULTS | DHW_DEFAULTS).items():
...
def validate(hass, values, current_entry_id=None):
    ...
    try:
        ...
        dhw = None
    for key, domains in (REQUIRED_ENTITIES | OPTIONAL_ENTITIES | DHW_ENTITIES).items():
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                values = {key: "" for key in OPTIONAL_ENTITIES | DHW_ENTITIES} | user_input
